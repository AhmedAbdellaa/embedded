...
uint8 LM_u8GetTemp(LM_Config_t* refrance_LM){
    ...
    if(refrance_LM != NULL ){
		...
		UART_VoidPrintChars("digSignal: ");
		UART_VoidPrintInt((sint32)(Local_u16Reading));
		UART_VoidPrintChars("	Temperature: ");
        //return temperature in Celsius
        refrance_LM->Temprature =  Local_u16VoltageReading/10 ;
        UART_VoidPrintFloat((f64)Local_u16VoltageReading/10.0);
        UART_voidTransmit(10);//new line
    }else{
