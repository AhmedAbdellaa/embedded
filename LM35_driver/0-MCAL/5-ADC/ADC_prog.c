...
uint8 ADC_u16startConverstionSync(ADC_channel_t copy_Channel, uint16* referance_u16DigResult)
{
	...
	if (referance_u16DigResult != NULL)
	{
		if(ADC_u8BusyFlag ==IDLE){
			...
			//&& Local_u32Counter < ADC_u32TIMEOUT_COUNT
...
uint8 ADC_u16startConverstionAsync(ADC_channel_t copy_Channel,uint16* referance_u16DigResult,void (*copy_pvNotificationFunc)(void)){
	uint8 Local_u8ErrorState = OK;
...
uint8 ADC_u8StartChainConverstionAsync(ADC_CHAIN_config_t* referance_adcChain,uint8 copy_u8NumberOfADC){
	...
	if (referance_adcChain != NULL )
	{
		...
		ADC_u16startConverstionAsync(referance_adcChain[ADC_u8ChainCounter].ConversionChannel,
...
void ADC_voidChainNotification(){
	...
	if (ADC_u8ChainCounter == ADC_u8ChainDeviceCount){
		...
		ADC_u16startConverstionAsync(ADC_u8ChainDevices[ADC_u8ChainCounter].ConversionChannel,
