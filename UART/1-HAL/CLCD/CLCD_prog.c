...
//    if (Local_s8counter > CLCD_u8_LINE_SIZE )
...
//    else
    ...
    for(Local_s8PrintCounter = Local_s8counter-1 ;Local_s8PrintCounter>=Local_s8MFlag;Local_s8PrintCounter-- ){
    	CLCD_VoidSendData(Local_u8Buffer[Local_s8PrintCounter]);
    }
//	if (Local_s8MFlag ==0)
//		_helper_VoidReverseBuffer(ref_u8Buffer,0,Local_s8counter-1);
//    else
//    	_helper_VoidReverseBuffer(ref_u8Buffer,1,Local_s8counter-1);
	//return ;
}
...
void CLCD_VoidPrintFloat(sint32 copy_s32IntegerPart,uint32 copy_s32FloatingPart){
    _helper_VoidNumToStr(copy_s32IntegerPart);
    CLCD_VoidSendData('.');
    _helper_VoidNumToStr(copy_s32FloatingPart);
}
